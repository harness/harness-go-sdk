...
func (a *CloudCostClusterOrchestratorApiService) ToggleClusterOrchestratorState(ctx context.Context, accountIdentifier string, orchID string, disabled bool) (ToggleStateResponse, *http.Response, error) {
	var (
		localVarHttpMethod  = strings.ToUpper("Put")
		localVarPostBody    interface{}
		localVarFileName    string
		localVarFileBytes   []byte
		localVarReturnValue ToggleStateResponse
	)

	localVarPath := fmt.Sprintf("%s/lw/api/accounts/%s/clusters/orchestrator/%s/toggle_state", a.client.cfg.BasePath, accountIdentifier, orchID)
	...
	localVarQueryParams.Add("accountIdentifier", parameterToString(accountIdentifier, ""))
	localVarQueryParams.Add("disabled", parameterToString(disabled, ""))
	// to determine the Content-Type header
	localVarHttpContentTypes := []string{"application/json"}
